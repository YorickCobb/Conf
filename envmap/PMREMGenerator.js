 ...
 */
 ...
 //customized to increase sample count - we don't need this to run in real time
 //																-Yorick
...
THREE.PMREMGenerator = ( function () {
	...
	var PMREMGenerator = function ( sourceTexture, samplesPerLevel, resolution ) {
		...
		this.samplesPerLevel = 8192; //increase this number for better blurring on rough surfaces (but going much higher than this will impact performance) -yorick
			...
			fragmentShader:
				...
				void main() {\n\
					...
					for( int i = 0; i < NumSamples; i ++ ) {\n\
						...
						weight += dotProd;\n\
						vec3 color = RGBEToLinear(textureCube(envMap, vect)).rgb;\n\
						rgbColor.rgb += color;\n\
					}\n\
